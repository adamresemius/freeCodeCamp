...
Use dot notation to set the `onclick` property of your `button1` to the function `goStore`. This function will be something you write later. Note that `button1` is already declared, so you do not need to use `let` or `const`.
